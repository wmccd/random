...
internal interface AlbumsStore {
    @Query("SELECT * FROM ${DatabaseConstants.AlbumsDatabase.Tables.Albums.NAME} ORDER BY albumArtist DESC")
    fun fetchAll(): Flow<List<AlbumEntity>>

    @Query("SELECT DISTINCT albumArtist FROM ${DatabaseConstants.AlbumsDatabase.Tables.Albums.NAME} ORDER BY albumArtist DESC")
    fun fetchAllArtists(): Flow<List<String>>

    @Query("SELECT * FROM ${DatabaseConstants.AlbumsDatabase.Tables.Albums.NAME} WHERE albumArtist < :albumArtist ORDER BY albumName DESC")
    ...
    suspend fun deleteByTripId(albumId: Int): Int
